...
// Récupérer la commande temporaire de l'utilisateur
...
$assocMail = [];
$assocMail['client'][$id_client] = $client ?: ['mail' => '-'];

// Tables nécessaires à la configuration du produit
$tables = ['structure', 'type_banquette', 'mousse', 'couleur_bois', 'accoudoir_bois',
           'dossier_bois', 'couleur_tissu_bois', 'motif_bois', 'decoration'];

// Fonction générique pour récupérer les données de chaque table
...
// Récupérer les dimensions de la commande temporaire
if ($commande) {
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['comment'])) {
    if (!empty(trim($_POST["comment"]))) {
        ...
        if ($commande) {
            ...
            $stmt->execute([$commentaire, $id_commande]);
