...
$stmt = $pdo->query("SELECT * FROM dossier_bois WHERE visible = 1 ORDER BY prix ASC");
  ...
  <main>
    ...
    <script>
      function retourEtapePrecedente() {
        const selectedAccoudoirs = localStorage.getItem('selectedAccoudoirBois');

        // S'il y a au moins un accoudoir sélectionné (id(s) présents dans le localStorage)
        if (selectedAccoudoirs && selectedAccoudoirs.trim() !== '') {
          window.location.href = "etape5-bois-accoudoir-place.php";
        } else {
          // Aucun accoudoir sélectionné
