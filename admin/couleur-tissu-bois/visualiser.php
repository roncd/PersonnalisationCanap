...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id'], $_POST['visible'])) {
    ...
    $stmt = $pdo->prepare("UPDATE couleur_tissu_bois SET visible = :visible WHERE id = :id");
    $stmt->execute([
        ':visible' => $visible,
        ':id' => $id
    ]);
    ...
    <main>
                    ...
                    <tbody>
                        ...
                        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                           ...
                           $checked = $row['visible'] ? 'checked' : '';
                            ...
                            echo "<td>{$row['nom']}</td>";
                            echo "<td>{$row['prix']}</td>";
                            echo "<td><img src='../uploads/couleur-tissu-bois/{$row['img']}' alt='{$row['nom']}' ></td>";
        ...
        <div id="supprimer-popup" class="popup">
            <div class="popup-content">
                ...
                <p>(L'élément sera supprimé définitivementet les kits coussins <br>associés à ce motif ne seront plus visible sur le site)</p>
                <br>
