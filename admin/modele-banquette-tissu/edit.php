    ...
    if (empty($nom) || !isset($prix)) {
        ...
        if (!isset($_SESSION['message'])) {
            try {
                ...
                $stmt = $pdo->prepare("UPDATE modele SET nom = ?, prix = ?, img = ?, visible = ? WHERE id = ?");
                ...
                if ($stmt->rowCount() > 0) {
                    ...
                    if ($newImageUploaded) {
                        $uploadPath = $uploadDir . $fileName;
                        $ancienneImagePath = '../uploads/modele/' . $ancienneImage;
                ...
                <form action="edit.php?id=<?= $id ?>" method="POST" enctype="multipart/form-data" class="formulaire-creation-compte">
                    ...
                    <div class="form-row">
                        <div class="form-group btn-slider">
                            ...
                            <label class="switch">
                                <input type="checkbox" id="visible" name="visible" <?php if ($modele['visible']) echo 'checked'; ?>>
