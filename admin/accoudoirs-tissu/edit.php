    ...
    if (empty($nom) || !isset($price)) {
        ...
        if (!isset($_SESSION['message'])) {
            try {
                ...
                $stmt = $pdo->prepare("UPDATE accoudoir_tissu SET nom = ?, prix = ?, img = ?, visible = ? WHERE id = ?");
                $stmt->execute([$nom, $price, $fileName, $visible, $id]);
                    ...
                    if ($newImageUploaded) {
                        $uploadPath = $uploadDir . $fileName;
                        $ancienneImagePath = '../uploads/accoudoirs-tissu/' . $ancienneImage;
                ...
                <form action="edit.php?id=<?php echo $accoudoirtissu['id']; ?>" method="POST" enctype="multipart/form-data" class="formulaire-creation-compte">
                    ...
                    <div class="form-row">
                        <div class="form-group btn-slider">
                            ...
                            <label class="switch">
                                <input type="checkbox" id="visible" name="visible" <?php if ($accoudoirtissu['visible']) echo 'checked'; ?>>
