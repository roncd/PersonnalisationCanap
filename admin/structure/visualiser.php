...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id'], $_POST['visible'])) {
    ...
    $stmt = $pdo->prepare("UPDATE structure SET visible = :visible WHERE id = :id");
    $stmt->execute([
        ':visible' => $visible,
        ':id' => $id
    ]);
        ...
        <div class="container">
            ...
            <div class="option">
                <div class="section-button">
                    ...
                    <div>
                        <button onclick="location.href='add.php'" class="btn-noir" type="button">+ Ajouter une structure</button>
                    </div>
                </div>
            ...
            <div class="tab-container">
                <table class="styled-table">
                    ...
                    <tbody>
                        ...
                        if ($search) {
                            $stmt = $pdo->prepare("SELECT * FROM structure WHERE nom LIKE :search ORDER BY id $order");
                            $stmt->bindValue(':search', '%' . $search . '%', PDO::PARAM_STR);
                        } else {
                            $stmt = $pdo->prepare("SELECT * FROM structure ORDER BY id $order LIMIT :limit OFFSET :offset");
