...
$stmt = $pdo->prepare("SELECT * FROM dossier_bois WHERE  id = :id");
    ...
    if (empty($nom) || !isset($prix)) {
        ...
        if (!empty($img['name'])) {
            ...
            if (!in_array($img['type'], $allowedTypes)) {
                ...
                $uploadDir = '../uploads/dossier-bois/'; // Dossier pour les dossiers de tissu
                if (!is_dir($uploadDir)) {
            ...
            try {
                ...
                $stmt->execute([$nom, $prix, $fileName, $visible, $id]);
                  ...
                  if ($stmt->rowCount() > 0) {
                    ...
                    if ($newImageUploaded) {
                        $uploadPath = $uploadDir . $fileName;
                        $ancienneImagePath = '../uploads/dossier-bois/' . $ancienneImage;
