...
$stmt = $pdo->prepare("SELECT * FROM sections_catalogue WHERE id = :id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($slug) || empty($titre)) {
        ...
        try {
            $stmt = $pdo->prepare("UPDATE sections_catalogue SET slug = ?, titre = ?, description = ?, bouton_texte = ?, bouton_lien = ?, img = ?, visible = ? WHERE id = ?");
...
<head>
    ...
    <title>Modifie une section - Catalogue</title>
...
<body>
    ...
    <main>
        <div class="container">
            <h2>Modifie une section - Catalogue</h2>
