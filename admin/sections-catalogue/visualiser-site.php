...
$stmtCount = $pdo->prepare("SELECT COUNT(*) AS total FROM sections_catalogue");
$stmtCount->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id'], $_POST['visible'])) {
    ...
    $stmt = $pdo->prepare("UPDATE sections_catalogue SET visible = :visible WHERE id = :id");
    $stmt->execute([
        ':visible' => $visible,
        ':id' => $id
    ]);
...
<head>
    ...
    <title>Sections - Catalogue</title>
        ...
        <div class="container">
            ...
            <div class="option">
                <div class="section-button">
                    ...
                    <div>
                        <button onclick="location.href='add-site.php'" class="btn-noir" type="button">+ Ajouter une section</button>
                    </div>
                </div>
            ...
            <div class="tab-container">
                <table class="styled-table">
                    ...
                    <tbody>
                        <?php
                        if ($search) {
                            $stmt = $pdo->prepare("SELECT * FROM sections_catalogue WHERE slug LIKE :search ORDER BY id $order");
                            $stmt->bindValue(':search', '%' . $search . '%', PDO::PARAM_STR);
                        } else {
                            $stmt = $pdo->prepare("SELECT * FROM sections_catalogue ORDER BY id $order LIMIT :limit OFFSET :offset");
                        ...
                        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                            ...
                            echo "<td>" . htmlspecialchars(mb_strimwidth($row['description'], 0, 100, '...')) . "</td>";
                            echo "<td><img src='../../medias/{$row['img']}' alt='{$row['slug']}' ></td>";
                            echo "<td>" . htmlspecialchars($row['bouton_texte']) . "</td>";
                            echo "<td>" . htmlspecialchars($row['bouton_lien']) . "</td>";
