...
$stmtCount = $pdo->prepare("SELECT COUNT(*) AS total FROM liste_dashboard");
$stmtCount->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id'], $_POST['visible'])) {
    ...
    $stmt = $pdo->prepare("UPDATE liste_dashboard SET visible = :visible WHERE id = :id");
    $stmt->execute([
        ':visible' => $visible,
        ':id' => $id
    ]);
...
<head>
    ...
    <title>Contenu de la liste - Dashboard</title>
...
<body>
    ...
    <main>
        <div class="container">
            <h2>Contenu de la liste - Dashboard</h2>
            <div class="option">
                <div class="section-button">
                    ...
                    <div>
                        <button onclick="location.href='add-site.php'" class="btn-noir" type="button">+ Ajouter une liste</button>
                    </div>
                </div>
            ...
            <div class="tab-container">
                <table class="styled-table">
                    ...
                    <tbody>
                        <?php
                        if ($search) {
                            $stmt = $pdo->prepare("SELECT * FROM liste_dashboard WHERE titre LIKE :search ORDER BY id $order");
                            $stmt->bindValue(':search', '%' . $search . '%', PDO::PARAM_STR);
                        } else {
                            $stmt = $pdo->prepare("SELECT * FROM liste_dashboard ORDER BY id $order LIMIT :limit OFFSET :offset");
                        ...
                        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                            ...
                            echo "<td><img src='../../assets/{$row['icon']}' alt='Icône' ></td>";
