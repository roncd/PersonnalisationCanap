...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id'], $_POST['visible'])) {
    ...
    $stmt = $pdo->prepare("UPDATE motif_tissu SET visible = :visible WHERE id = :id");
    $stmt->execute([
        ':visible' => $visible,
        ':id' => $id
    ]);
            ...
            <div class="tab-container">
                <table class="styled-table">
                    ...
                    <tbody>
                        <?php
                        if ($search) {
                            $stmt = $pdo->prepare("SELECT * FROM motif_tissu WHERE nom LIKE :search ORDER BY id $order");
                            $stmt->bindValue(':search', '%' . $search . '%', PDO::PARAM_STR);
                        } else {
                            $stmt = $pdo->prepare("SELECT * FROM motif_tissu ORDER BY id $order LIMIT :limit OFFSET :offset");
                        ...
                        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                           ...
                           $checked = $row['visible'] ? 'checked' : '';
                            ...
                            echo "<td>{$row['nom']}</td>";
                            echo "<td>{$row['prix']}</td>";
                            echo "<td><img src='../uploads/motif-tissu/{$row['img']}' alt='{$row['nom']}' ></td>";
