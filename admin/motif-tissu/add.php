    ...
    if (empty($nom) || !isset($prix) || empty($img['name']) || empty($id_couleur)) {
        ...
        if (!in_array($img['type'], $allowedTypes)) {
            ...
            if (move_uploaded_file($img['tmp_name'], $uploadPath)) {
                try {
                    $stmt = $pdo->prepare("INSERT INTO motif_tissu (nom, prix, img, id_couleur_tissu, visible) VALUES (?, ?, ?, ?, ?)");
                    $stmt->execute([$nom, $prix, $fileName, $id_couleur, $visible]);
...
<head>
    ...
    <title>Ajoute un Kit de coussins - tissu</title>
...
<body>
    ...
    <main>
        <div class="container">
            <h2>Ajouter un Kit de coussins - tissu</h2>
