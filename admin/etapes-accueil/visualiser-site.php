...
$stmtCount = $pdo->prepare("SELECT COUNT(*) AS total FROM etapes_accueil");
$stmtCount->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id'], $_POST['visible'])) {
    ...
    $stmt = $pdo->prepare("UPDATE etapes_accueil SET visible = :visible WHERE id = :id");
    $stmt->execute([
        ':visible' => $visible,
        ':id' => $id
    ]);
        ...
        <div class="container">
            ...
            <div class="option">
                <div class="section-button">
                    ...
                    <div>
                        <button onclick="location.href='add-site.php'" class="btn-noir" type="button">+ Ajouter une étape</button>
                    </div>
                </div>
            ...
            <div class="tab-container">
                <table class="styled-table">
                    ...
                    <tbody>
                        <?php
                        if ($search) {
                            $stmt = $pdo->prepare("SELECT * FROM etapes_accueil WHERE titre LIKE :search ORDER BY id $order");
                            $stmt->bindValue(':search', '%' . $search . '%', PDO::PARAM_STR);
                        } else {
                            $stmt = $pdo->prepare("SELECT * FROM etapes_accueil ORDER BY id $order LIMIT :limit OFFSET :offset");
                        ...
                        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                            ...
                            echo "<td>" . htmlspecialchars($row['numero_etape']) . "</td>";
